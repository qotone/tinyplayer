...
static FRESULT scan_files(char* path, uint16_t tmp_buffer_size, Q_FATFS_Search_t* FindStructure);
...
FRESULT Q_FATFS_GetDriveSize(char *str, Q_FATFS_Size_t *SizeStruct) {
  ...
  SizeStruct->TotalSize = (fs->n_fatent -2) * fs->csize  / 2 ;
  SizeStruct->FreeSize = fre_clust * fs->csize / 2;
...
uint8_t Q_FATFS_CheckCardDetectPin(void) {
	...
	//add some code in this area. if want.
#endif
...
/*******************************************************************/
__weak uint8_t Q_FATFS_SearchCallback(char* path, uint8_t is_file, Q_FATFS_Search_t* FindStructure) {
	...
	if ((res = f_opendir(&dir, path)) == FR_OK) {
		...
		while ((res = f_readdir(&dir, &fno)) == FR_OK && fno.fname[0] != 0) {
			...
			if (fno.fattrib & AM_DIR) {
				...
				/* Call user function */
				gonext = Q_FATFS_SearchCallback(path, 0, FindStructure);
				...
				/* Call user function */
				gonext = Q_FATFS_SearchCallback(path, 1, FindStructure);
			}
