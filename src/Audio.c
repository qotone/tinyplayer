...
extern QueueHandle_t mp3DataQueue;
extern SemaphoreHandle_t mp3DecodeCompleteSem ;
extern SemaphoreHandle_t mp3DecodeStopSem;
/*******************************************************************************
...
void PlayAudioFile(FIL *FileObject,char *path)
{
   ...
   {
      strcpy( mp3_info.title, "no file title");  
   }

   if ( strlen(mp3_info.title) > 20 )
   {
      strcpy( mp3_info.title, "file title too long");  
   }
   ...
   while( ( res == FR_OK && n_Read != 0 ) )// && !OSSemAccept(StopMP3Decode) )
   {
	   ...
	   if(xSemaphoreTake(mp3DecodeStopSem,0) == pdTRUE)
	   {
		   stopflag = 1;
		   break;
	   }
       res = f_read( FileObject, readBuf,sizeof(readBuf) , &n_Read );
       MP3_Data_Index += n_Read;             /* 记录已播放大小 */
       //printf("read %d bytes\r\n",n_Read);
       buf = readBuf;
	   
       //	   OSMboxPost(mp3Data,(void*)readBuf );  /* 传送数据解码 */
       result = xQueueSend(mp3DataQueue,&buf,(TickType_t)100);
//       if(result == pdTRUE)
//    	   printf("ok\r\n");
//       else
//    	   printf("error\r\n");
       ...
       xSemaphoreTake(mp3DecodeCompleteSem,portMAX_DELAY);
              
   } 

   if((res == FR_OK && n_Read == 0 )|| stopflag)
   {
	  stopflag = 0;
      outOfData = 1;  
      AUDIO_Playback_Stop();
